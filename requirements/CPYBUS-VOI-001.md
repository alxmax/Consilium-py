...
- `call_voice(_voice_name, system_prompt, user_msg, model)` shall call `client.messages.create` with `max_tokens=4096` and return the text of the first `TextBlock` in the response. If no `TextBlock` is present, it returns an empty string.
...
- Should `call_voice` retry on transient API errors (rate-limit, timeout), or is that the caller's responsibility? Currently no retry logic exists.
