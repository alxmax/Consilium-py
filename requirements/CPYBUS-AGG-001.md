...
- `aggregate_sequential(cons_text, gen_text, ctrl_text, _inp)` shall parse each text argument as JSON (via `extract_json`) and run the veto cascade in this fixed priority order:
  ...
  3. Generator `abstain.triggered` with `abstain.reason == "not_a_proposal"` → `BLOCK` (the input is not a code-change or decision proposal — nothing to deliberate). This is distinct from the three *soft* abstain reasons (`contradiction`, `no_data`, `goal_undefined`), which do not short-circuit and instead discount `confidence_methodology` on the `AGGREGATE` path.
  ...
  6. `scale_down` meta-recommendation → `ADAPT_SHORT` → mapped to `GO`
  7. `scale_up` meta-recommendation → `ADAPT_EXTENDED` → mapped to `MODIFY`
  8. Default → `AGGREGATE` (normal scoring)
